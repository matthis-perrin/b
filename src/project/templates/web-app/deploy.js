...
function ensureDistFolders() {
  ...
  if (backendFiles.length === 0) {
    writeFileSync(path.join(backendDist, 'main.js'), `exports.handler = async function() {return ''};`);
  }
}
...
async function run() {
  ...
  // Build the frontend
  runCommand({command: `yarn build`, cwd: frontendPath, env: {...process.env, PUBLIC_PATH: `https://${outputs.cloudfront_domain_name.value}`}});
  const INDEX_HTML = readFileSync(path.join(frontendDist, 'index.html')).toString()

  // Build the backend
  runCommand({command: `yarn build`, cwd: backendPath, env: {...process.env, MATTHIS_INDEX_HTML: JSON.stringify(INDEX_HTML)}});
